...
contract Samudai is Ownable {
	...
	bytes32 public domainHash = 0x4a63d36066671308df5b4368b460753f2ca0ef94f3295ba5d604193945284928;
	string public domainLabel = "samudai";
	...
	function claimSubdomain(string calldata label, string memory message, bytes memory signature) public isAuthorised(message, signature) {
	...
	) external isAuthorised(message, signature) {
		address currentAddress = hashToAddressMap[node];
		require(addressToHashmap[currentAddress] != 0x0, "Invalid address");
	...
	function resetHash(string memory message, bytes memory signature) public isAuthorised(message, signature)
    {
    ...
    function updateMerkleHash(bytes32 _hash) public onlyOwner {
        merkleHash = _hash;
    }
	...
	function isValid(string memory message, bytes memory signature) public view returns(bool) {
        bytes32 r;
        bytes32 s;
        uint8 v;
	...
	modifier isAuthorised(string memory message, bytes memory signature) {
		require(owner() == msg.sender || isValid(message, signature), "Unauthorised user");
		_;
	}
